...
#+begin_src latex
  ...
  We also build an lcp cache with the method \ty{buildLcpCache}, which
  we will define in Section~\ref{lcp_cache}. We cache intervals for
  6-mers by default.
#+end_src
#+begin_src go <<Functions>>=
...
#+begin_src latex
  ...
  !array, the length of cached k-mers and a cache of lcp match intervals.
...
#+begin_src latex
  ...
  \textit{H. influenze}, which has about 1.9 million
  ...
  into a cache, and look them up during prefix matching. The cache is an
  array that stores lcp intervals, \ty{lcpCache[i]}, where \ty{i} is an
  index corresponding to a $k$-mer. We will implement such a cache in
  this package. To do this, we have to solve two problems: find indexes
  (find and place all possible $k$-mers in the cache), and find lcp
  intervals.
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src latex
  \subsubsection{Find k-mer indexes}
  An input $k$-mer is an array of bytes. We want to convert this array
  into an integer index to an array. For this, we need to write a
...
#+end_src
#+begin_src latex
  Now we are ready to calculate cache indexes for $k$-mers. Consider the
  6-mer \ty{TGCATC}. If we encode the nucleotides as \ty{[A = 0, C = 1,
      G = 2, T = 3]}, we get the array of 2-bit codes \ty{[3, 2, 1, 0,
      3, 1]}. We want to turn this array into a single integer index. We
  take the initial index, zero, shift its bits to the left by 2 bits (in
  terms of decimal operations, quadruple it):
...
#+end_src
#+begin_src latex
  \subsubsection{Find k-mer LCP intervals}
  Now that we know how to index the cachde, we deal with cached
  values. Our goal is to find lcp intervals with longest matches for
  each $k$-mer.

  \newpage

  Consider the text \ty{ACGGGAAAGCGGAT} and its abridged ESA:
  ...
  Let's say we want to find lcp intervals for the 3-mer \ty{AAA}. We see
  in the ESA that there is a singleton interval \ty{[1..1]}, which holds
  the only suffix \ty{AAAGCGGAT}. The next the 3-mer \ty{AAC} does not
  match completely any of the suffixes. The longest matches for \ty{AAC}
  are found in the interval \ty{[1..2]}, that is, in the suffixes
  \ty{AAAGCGGAT} and \ty{AAGCGGAT}. Although our text does not contain
  suffixes starting with \ty{AAC}, a query absolutely can start with
  \ty{AAC}, so the partial match is still worth saving.


  We initialize a $k$-mer, find the root lcp interval of the ESA's lcp
  table and call the helper method \ty{walkKmerTrie}, which is not
  wrtitten yet.
#+end_src
...
#+begin_src latex
  ...
  output of \ty{GetInterval}: no such interval (the extension doesn't
  exist) or a singleton interval (further extension is not possible). We
  get the interval's path label length, and deal with further extensions
  (that is, there's a longer interval deeper in the tree). Then we deal
  with a long edge.
#+end_src
#+begin_src go <<Extend the k-mer with a nucleotide>>=
  ...
  //<<Get length of path label for \ty{ij}>>
  //<<Further extension is possible>>
  //<<Deal with a long edge>>
#+end_src
#+begin_src latex
If we find no matches for the current $k$-mer, this extension doesn’t
...
#+end_src
#+begin_src latex
  We determine the length of the path label of the child array.
#+end_src
#+begin_src go <<Get length of path label for \ty{ij}>>=
  i, j := ij.I, ij.J
  var mid int
  if e.Lcp[i] <= e.Lcp[j+1] {
	    mid = e.Cld[j]
...
#+begin_src latex
  ...
  one-letter extensions and fast-forward to the depth 5.

  We check if the long edge fits the cache. If so, we fast-forward the
  edge. If the edge doesn't fit the cache, we pre-fill all $k$-mer
  indexes under this extension with the parent lcp interval.
#+end_src
#+begin_src go <<Deal with a long edge>>=
  if ij.L >= kmerLen {
	  e.fillKmerSubtree(kmer, currentK + 1, iv)
	  continue
  }
...
#+end_src
#+begin_src latex
  We prefill the subtree with the parent interval.

  If the current match is longer than the possible $k$-mer extension, we
  copy the interval, define a condition to stop the fast-forwarding and
  copy known nucleotides from the long edge to the $k$-mer.
#+end_src
...
#+end_src
#+begin_src latex
  If we have encountered a non-ACGT byte on the edge, we write the child
  interval from the deepest valid point. Otherwise we call
  \ty{walkKmerTrie} recursively to continue usual traversal.
#+end_src
#+begin_src go <<Deal with non-ACGT>>=
  if nonACGT {
...
#+begin_src latex
  \section{Testing}
  The testing outline contains hooks for imports and the testing
  logic. We test the ESA implementation and LCP cache.
#+end_src
#+begin_src go <<esa_test.go>>=
  ...
  //<<Test ESA implementation>>
  //<<Test LCP cache>>
#+end_src
#+begin_src latex
  \subsection{Test ESA implementation}
...
#+end_src
#+begin_src go <<Test ESA implementation>>=
...
#+end_src
#+begin_src latex
  \subsection{Test LCP cache}
  We test lcp cache building.
#+end_src
#+begin_src go <<Test LCP cache>>=
  //<<Test LCP cache building>>
#+end_src
#+begin_src latex
  We build a small nucleotide suffix array, call the method for building
  an LCP cache. Then we lookup cache values.
#+end_src
#+begin_src go <<Test LCP cache building>>=
  func TestBuildLcpCache(t *testing.T) {
	  //<<Compute small nucleotide ESA>>
	  esa.buildLcpCache()
	  //<<Check the cache>>
  }
#+end_src
#+begin_src latex
  We create a 14-nucleotide string of nucleotides \ty{[A, C, G, T]},
  initialize the ESA struct and populate it with the text, SA, LCP, and
  CLD as we compute them. We set the k-mer length to 2 nucleotides, that
  is, we are going to cache lcp intervals for 16 2-mers.
#+end_src
#+begin_src go <<Compute small nucleotide ESA>>=
  text := "ACGGGAAAGCGGAT"
  var esa Esa
  esa.T = []byte(text)
  esa.T = append(esa.T, 0)
...
#+end_src
#+begin_src latex
  The ESA of our text looks like this:
  ...
  5  12 AT             1   3
  6  9  CGGAT          0   8
  7  1  CGGGAAAGCGGAT  3   7
  8  4  GAAAGCGGAT     0   14
  9  11 GAT            2   9
  10 8  GCGGAT         1   11
  11 3  GGAAAGCGGAT    1   13
  12 10 GGAT           3   12
  13 2  GGGAAAGCGGAT   2   10
  14 13 T              0   1
  \end{verbatim}
#+end_src
#+begin_src latex
  We define the desired cache values and compare what we want with what
  we get.
#+end_src
#+begin_src go <<Check the cache>>=
  //<<Define the cache we want>>
  get := esa.lcpCache
...
#+end_src
#+begin_src latex
  For 16 implied 2-mers \ty{["AA", "AC", ... "TT"]}, we expect to see
  the following indexes and intervals in the lcp cache:
  ...
  The interval \ty{[0..14]} corresponds to the root of the implied
  suffix tree and is saved for k-mers, which are not prefixes of any
  suffixes of the ESA.

  We create an array of 16 empty intervals and fill it with interval
  values from the table above. This is our desired cache.
#+end_src
#+begin_src go <<Define the cache we want>>=
  ivs := make([]Minterval, 16)
  starts := [16]int{1,3,4,5,0,0,0,0,8,10,11,8,14,14,14,14}
  ends := [16]int{2,3,4,5,14,14,14,14,9,10,13,13,14,14,14,14}
  lengths := [16]int{2,2,2,2,0,0,0,0,2,2,2,1,1,1,1,1}
